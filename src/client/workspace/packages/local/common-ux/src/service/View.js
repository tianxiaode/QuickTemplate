 ...
 * 视图管理类
 */
Ext.define('Common.service.View', {
    alternateClassName: 'ViewService',
    singleton: true,
     ...
     * @returns 无
     */
    show(xtype, type, config, hasDefault, container) {
        let me = this,
     ...
     */
    showPage(xtype) {
        this.show(xtype, this.types.view, null, false, Ext.Viewport);
    },
    ...
    doDestroy() {
        let me = this;
        me.destroyMembers('views', 'viewParams', 'types', 'defaultConfig', 'pages');
    ...
    privates: {
        ...
        getWidget(xtype) {
            if (Ext.isEmpty(xtype)) return null;
            let appName = this.getAppName().toLowerCase(),
