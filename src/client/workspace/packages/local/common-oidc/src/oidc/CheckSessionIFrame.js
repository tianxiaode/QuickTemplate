    ...
    privates:{
        message(e){
            ...
            if (e.origin === frameOrigin &&
                ...
                if (e.data === "error") {
                    Logger.error(me.message, "error message from check session op iframe");
                    if (me.stopOnError) {
                        me.stop();
                    }
                }
                else if (e.data === "changed") {
                    Logger.debug(me.message, "changed message from check session op iframe");
                    me.stop();
                    me.callback();
                }
                else {
                    Logger.debug(me.message, e.data + " message from check session op iframe");
                }
            }
